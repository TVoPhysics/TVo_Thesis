#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput VarySR3_95MM_wSqz_MMTTon_maxtem4.out
  created automatically Tue Sep 13 22:09:04 2016

  Run from command line as: python VarySR3_95MM_wSqz_MMTTon_maxtem4.py
  Load from python script as: import VarySR3_95MM_wSqz_MMTTon_maxtem4
  And then use:
  VarySR3_95MM_wSqz_MMTTon_maxtem4.run() for plotting only
  x,y=VarySR3_95MM_wSqz_MMTTon_maxtem4.run() for plotting and loading the data
  x,y=VarySR3_95MM_wSqz_MMTTon_maxtem4.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('VarySR3_95MM_wSqz_MMTTon_maxtem4.out',comments='%')
	rows,cols=data.shape
	x=data[:,0]
	y=data[:,1:cols]
	mytitle='VarySR3_95MM_wSqz_MMTTon_maxtem4                Tue Sep 13 22:09:04 2016'
	if (noplot==None):
