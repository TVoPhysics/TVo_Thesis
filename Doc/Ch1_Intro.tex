\chapter{Introduction}
	...
	How to practically measure $\Delta \Phi$ with an interferometer is explained in the next section.
	...
	The LIGO instruments are considered dual-recycled Fabry-Perot interferometers.
		...
		The last term is referred to as the $beat$ $note$ between the carrier signal and the sidebands.  It is possible to extract only the signal at the modulation frequency using a mixer and low-pass filter to get the demodulated signal
		
		\begin{equation}
		S = 4 \pi  J_0(\beta) J_1(\beta) \frac{\ell}{\lambda}  \sin(k_{\Omega} \Delta \ell)  \; h_{+}
		\end{equation}
		
		This shows that a RF detection technique will be linear in GW signal with no large DC offset.  However, this method depends on the local oscillator's stability. 
		
		Setting the carrier on the null point means $\Delta \ell = \frac{k_{\Omega}}{k} \frac{\pi}{2}$ and allows the designer to optimize the Schnupp asymmetry length to get the best signal for some modulation frequency.
