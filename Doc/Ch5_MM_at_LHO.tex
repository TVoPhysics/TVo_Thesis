\chapter{Mode Matching Cavities at LIGO Hanford}
	...
	All estimates of steady-state curvature changes due to heating by the main interferometer beam depend linearly on the absorption and this can be quite difficult to detect when the coating absorption estimated to range between 0.2 and 0.5 PPM.  In order to diagnose this effect, the Hartmann Wavefront Sensors (HWS) was a system developed by Adelaide and Caltech which uses a charged coupled imaging device (CCD) to sense the wavefront distortions created by heating.
